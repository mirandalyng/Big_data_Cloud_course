...
@dp.table(name="supply_chain_live.bronze.raw_supply_chain", comment ="Raw supply chain data for DataCo", table_properties={
...
def raw_supply_chain(): 
    return (
        spark.readStream.format("csv").options(header=True, encoding="latin1").schema(schema).load(f"{BASE_DIR}/data")
    )
